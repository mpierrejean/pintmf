...
fig_height: 10
vignette: >
%\VignetteIndexEntry{MultiOM vignette}
...
```
## Figures Raw data
```{r, heatmap_plot, fig.width=10, fig.height=5}
cols.clust <- RColorBrewer::brewer.pal(4, "Set1")[true.clust %>% as.factor] 
data[[1]] %>%  gplots::heatmap.2( dendrogram="both", trace="none",Rowv=TRUE, RowSideColors=cols.clust, scale="none")
data[[2]] %>%  gplots::heatmap.2( dendrogram="both", trace="none",Rowv=TRUE, RowSideColors=cols.clust, scale="none")
data[[3]] %>%  gplots::heatmap.2( dendrogram="both", trace="none",Rowv=TRUE, RowSideColors=cols.clust, scale="none")
```
...
R_moclust <- CrIMMix::IntMultiOmics(data_t, method="Mocluster", K=4, k= c(0.4, 0.2, 0.6))
R_intNMF <- CrIMMix::IntMultiOmics(data_t, method="intNMF", K=4)

R_SNF <- CrIMMix::IntMultiOmics(data_t, method="SNF", K=4)
```
...
cat("my clustering:", mclust::adjustedRandIndex(clust, true.clusters), "\n")
cat("mocluster clustering:", mclust::adjustedRandIndex(clust_moclust, true.clusters), "\n")
cat("intNMF clustering:", mclust::adjustedRandIndex(clust_intNMF, true.clusters), "\n")
cat("SNF clustering:", mclust::adjustedRandIndex(clust_snf, true.clusters), "\n")
cols <- RColorBrewer::brewer.pal(4, "Set1")
table(true.clusters, clust)
library(RColorBrewer)
pal <- rev(RColorBrewer::brewer.pal(10, "RdBu"))
gplots::heatmap.2(R$W, scale="none", trace="none",RowSideColors = cols[as.factor(true.clusters)],
                  col = pal)
```
...
est_muclust <- lapply(1:ndat, function (kk){
  ...
  regexp <- "[[:digit:]]+"
  gsub(sprintf("_dat%s", kk), "", names(est_muclust)[idx]) %>% stringr::str_extract(pattern=regexp)
})  
...
```{r,AUCS, fig.width=7}
auc <- function (x, y){
  round(sum(tis::lintegrate(c(x,1), c(y,1), xint=c(x,1))),2)
}
...
df_ARI <- data.frame(method=c("my_method", "mocluster", "NMF", "SNF"), ari=c(mclust::adjustedRandIndex(clust, true.clusters), mclust::adjustedRandIndex(clust_moclust, true.clusters), mclust::adjustedRandIndex(clust_intNMF, true.clusters), 
                                                                             mclust::adjustedRandIndex(clust_snf, true.clusters)))
